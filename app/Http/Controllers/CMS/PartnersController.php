...
class PartnersController extends Controller
{
    ...
    {
        try {
            return Inertia::render('CMS/Partners/CreatePartner');
        } catch (\Throwable $ex) {
            logError($ex);
    ...
    protected function createPost(PartnerFormRequest $request)
    {
        try {
            DB::beginTransaction();
            return redirect(route('partners-management-page'));
        } catch (\Throwable $ex) {
            DB::rollback();
            logError($ex);
    ...
    {
        try {
            return Inertia::render('CMS/Partners/PartnerEditor');
        } catch (\Throwable $ex) {
            DB::rollback();
            logError($ex);
            if ($ex instanceof AuthorizationException) {
    ...
    protected function editPost(PartnerFormRequest $request)
    {
        try {
            DB::beginTransaction();
            return redirect(route('partners-management-page'));
    ...
    protected function manage()
    {
        try {
            ...
            return Inertia::render('CMS/Partners/PartnerManagement', $data);
        } catch (\Throwable $ex) {
            logError($ex);
            if ($ex instanceof AuthorizationException) {
                abort(403, getUnAuthorizedAccessMessage());

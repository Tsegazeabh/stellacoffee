...
class Cafe extends Authorizable implements HasMedia, Auditable
{
    ...
    public function registerMediaCollections(): void
    {
        $this
            ...
            ->registerMediaConversions(function ( Media $media ) {
                $this->addMediaConversion('cafe-images')
    ...
    {
        if (!empty($this->hasMedia('file-attachments')) && $this->getMedia('file-attachments')[0]->hasGeneratedConversion('cafe-images')) {
            return $this->getFirstMedia('file-attachments')->responsive_images["cafe-images"];
        }
    ...
    public function getSrcSetsAttribute()
    {
        ...
        for ($i = 0; $i < $this->media()->count(); ++$i) {
            array_push($srcSets, $this->getMedia('file-attachments')[$i]->getSrcset('cafe-images'));
        }
        return $srcSets;
    }
    ...
    public function scopeSortBy($query, $sorting_column, $sorting_method)
    {
        switch ($sorting_column) {
            case 'created_at':
                return $query->where('contentable_type', Cafe::class)->join('Contents as c', 'c.contentable_id', '=', $this->getTable() . '.id')->orderBy('c.created_at', $sorting_method);
                break;
            case 'published_at':
                return $query->where('contentable_type', Cafe::class)->join('Contents as c', 'c.contentable_id', '=', $this->getTable() . '.id')->orderBy('c.published_at', $sorting_method);
                break;
            case 'author':
                return $query->where('contentable_type', Cafe::class)->join('Contents as c', 'c.contentable_id', '=', $this->getTable() . '.id')->orderBy('c.created_by_id', $sorting_method);
                break;
            case 'archived_date':
                break;
            case 'content_status':
                return $query->where('contentable_type', Cafe::class)->join('Contents as c', 'c.contentable_id', '=', $this->getTable() . '.id')->orderBy('c.is_published', $sorting_method)->orderBy('deleted_at', $sorting_method);
                break;
            default:

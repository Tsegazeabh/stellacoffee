...
class News extends Authorizable implements Auditable
{
    ...
    public function scopeSortBy($query, $sorting_column, $sorting_method)
    {
        ...
        switch ($sorting_column) {
            case 'created_at':
                return $query->where('contentable_type', News::class)->join($contentsTable . ' as c', 'c.contentable_id', '=', $this->getTable() . '.id')->orderBy('c.created_at', $sorting_method);
                break;
            case 'published_at':
                return $query->where('contentable_type', News::class)->join($contentsTable . ' as c', 'c.contentable_id', '=', $this->getTable() . '.id')->orderBy('c.published_at', $sorting_method);
                break;
            case 'deleted_at':
                break;
            case 'content_status':
                return $query->where('contentable_type', News::class)->join($contentsTable . ' as c', 'c.contentable_id', '=', $this->getTable() . '.id')->orderBy('c.is_published', $sorting_method)->orderBy('deleted_at', $sorting_method);
                break;
            default:

...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const onEditorReady = (editor) => {
            ...
            if (props.export_process) {
                form.value.detail = props.export_process.detail
            }
        };
                ...
                onSuccess: (page) => {
                    if (Object.keys(page.props.errors).length == 0) {
                        if (props.method.toLowerCase() == 'put') {
                            $toast.success('Export process is updated successfully!')
                        } else {
                            $toast.success('Export process is created successfully!')
                        }
                        form.value.errors = [];
                        form.value.reset();
                    } else {
        ...
        onMounted(() => {
            if (props.export_process) {
                form.value.title = props.export_process.title
                form.value.video_link = props.export_process.video_link
            }
            if (props.export_process && props.export_process.content && props.export_process.content.tags) {
                form.value.tags = props.export_process.content.tags
            } else if (props.export_process && props.export_process.content) {
                form.value.tags = []
            }
        });
        return {

                ...
                <div class="w-full md:w-1/2">
                    <div class="form-group w-full px-5">
                        ...
                        <input type="text" v-model.trim.lazy="form.shop_address"
                               ...
                               :placeholder="_trans('label.shared.Shop Address')"
                               autocomplete="on" required/>
                        <span class="text-red-500 font-semibold mt-3"
                              v-if="form.errors && form.errors['shop_address']">
...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const onEditorReady = (editor) => {
            ...
            if (props.shop) {
                form.value.detail = props.shop.detail
            }
        };
        ...
        onMounted(async () => {
            await loadCountries();
            form.value.title = props.shop.title
            form.value.airport_name = props.shop.airport_name
            form.value.shop_address = props.shop.shop_address
            form.value.country_id = props.shop.country_id
            form.value.city_id = props.shop.city_id
            form.value.longitude = props.shop.longitude
            form.value.latitude = props.shop.latitude
            form.value.video_link = props.shop.video_link

            if (props.shop && props.shop.content && props.shop.content.tags) {
                form.value.tags = props.shop.content.tags
            } else if (props.shop && props.shop.content) {
                form.value.tags = []
            }

            if(props.shop && props.shop.country_id){
                await loadCities();
            }
        });

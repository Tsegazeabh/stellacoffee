<template>
    <div class="flex flex-col mb-5 pb-5" id="most-popular-contents-container">
        ...
        <template v-if="contents && contents.length >0">
            <inertia-link v-for="(content,index) of contents" :key="content.id" :href="content.url" class="text-gray-600 flex ml-3 py-3 hover:text-black" :class="(index!=contents.length-1)?'border-b border-gray-100':''">

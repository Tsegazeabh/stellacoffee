                ...
                <div class="w-full md:w-1/2">
                    <div class="form-group w-full px-5">
                        ...
                        <input type="text" v-model.trim.lazy="form.title"
                               ...
                               :placeholder="_trans('label.shared.Title')" autocomplete="form.title"/>
    ...
    export default {
        ...
        mounted() {
            if (this.news) {
                ...
                this.form.video_link = this.news.video_link
            }
            if (this.news && this.news.content && this.news.content.tags) {
                this.form.tags = this.news.content.tags
            } else if (this.news && this.news.content) {
                this.form.tags = []
            }
        },
        methods: {
            onEditorReady(editor) {
                ...
                if (this.news) {
                    this.form.detail = this.news.detail
                }
            },
            save() {
                    ...
                    onSuccess: (page) => {
                        if (Object.keys(page.props.errors).length == 0) {
                            if (this.method.toLowerCase() == 'put') {
                                this.$toast.success('News is updated successfully!')
                            } else {
                                this.$toast.success('News is created successfully!')
                            }
                            this.form.errors = [];
                            this.form.reset();
                        } else {

...
export default defineComponent({
    ...
    mounted() {
        if (this.product_blend) {
            this.form.title = this.product_blend.title
            this.form.video_link = this.product_blend.video_link
        }
        if (this.product_blend && this.product_blend.content && this.product_blend.content.tags) {
            this.form.tags = this.product_blend.content.tags
        } else if (this.product_blend && this.product_blend.content) {
            this.form.tags = []
        }
    },
    methods: {
        onEditorReady(editor) {
            ...
            if (this.product_blend) {
                this.form.detail = this.product_blend.detail
            }
        },
        save() {
                ...
                onSuccess: (page) => {
                    if (Object.keys(page.props.errors).length == 0) {
                        if (this.method.toLowerCase() == 'put') {
                            this.$toast.success('Product Blend is updated successfully!')
                        } else {
                            this.$toast.success('Product Blend is created successfully!')
                        }
                        this.form.errors = [];
                        this.form.reset();
                    } else {

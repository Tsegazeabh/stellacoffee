...
export default defineComponent({
    ...
    mounted() {
        if (this.roasting_machine) {
            this.form.title = this.roasting_machine.title
            this.form.video_link = this.roasting_machine.video_link
        }
        if (this.roasting_machine && this.roasting_machine.content && this.roasting_machine.content.tags) {
            this.form.tags = this.roasting_machine.content.tags
        } else if (this.roasting_machine && this.roasting_machine.content) {
            this.form.tags = []
        }
    },
    methods: {
        onEditorReady(editor) {
            ...
            if (this.roasting_machine) {
                this.form.detail = this.roasting_machine.detail
            }
        },
        save() {
                ...
                onSuccess: (page) => {
                    if (Object.keys(page.props.errors).length == 0) {
                        if (this.method.toLowerCase() == 'put') {
                            this.$toast.success('Roasting Machine is updated successfully!')
                        } else {
                            this.$toast.success('Roasting Machine is created successfully!')
                        }
                        this.form.errors = [];
                        this.form.reset();
                    } else {

...
<script>
    ...
    export default {
        ...
        mounted() {
            if (this.roasting_process) {
                this.form.title = this.roasting_process.title
                this.form.video_link = this.roasting_process.video_link
            }
            if (this.roasting_process && this.roasting_process.content && this.roasting_process.content.tags) {
                this.form.tags = this.roasting_process.content.tags
            } else if (this.roasting_process && this.roasting_process.content) {
                this.form.tags = []
            }
        },
        methods: {
            onEditorReady(editor) {
                ...
                if (this.roasting_process) {
                    this.form.detail = this.roasting_process.detail
                }
            },
            save() {
                    ...
                    onSuccess: (page) => {
                        if (Object.keys(page.props.errors).length == 0) {
                            if (this.method.toLowerCase() == 'put') {
                                this.$toast.success('Roasting Process is updated successfully!')
                            } else {
                                this.$toast.success('Roasting Process is created successfully!')
                            }
                            this.form.errors = [];
                            this.form.reset();
                        } else {

...
<script>
    ...
    export default {
        ...
        name: "StellaCoffeeOriginForm",

        props: {
            method: {type: String, default: 'post'},
            url: {type: String, required: true},
            stella_coffee_origin: {
                type: Object,
                default: {
                    title: '',
                    detail: '',
        ...
        mounted() {
            if (this.stella_coffee_origin) {
                this.form.title = this.stella_coffee_origin.title
                this.form.video_link = this.stella_coffee_origin.video_link
            }
            if (this.stella_coffee_origin && this.stella_coffee_origin.content && this.stella_coffee_origin.content.tags) {
                this.form.tags = this.stella_coffee_origin.content.tags
            } else if (this.stella_coffee_origin && this.stella_coffee_origin.content) {
                this.form.tags = []
            }
        },
        methods: {
            onEditorReady(editor) {
                ...
                if (this.stella_coffee_origin) {
                    this.form.detail = this.stella_coffee_origin.detail
                }
            },
            save() {
                    ...
                    onSuccess: (page) => {
                        if (Object.keys(page.props.errors).length == 0) {
                            if (this.method.toLowerCase() == 'put') {
                                this.$toast.success('Stella Coffee Origin is updated successfully!')
                            } else {
                                this.$toast.success('Stella Coffee Origin is created successfully!')
                            }
                            this.form.errors = [];
                            this.form.reset();
                        } else {

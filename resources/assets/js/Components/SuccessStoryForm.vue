...
export default defineComponent({
    ...
    mounted() {
        if (this.success_story) {
            this.form.title = this.success_story.title
            this.form.video_link = this.success_story.video_link
        }
        if (this.success_story && this.success_story.content && this.success_story.content.tags) {
            this.form.tags = this.success_story.content.tags
        } else if (this.success_story && this.success_story.content) {
            this.form.tags = []
        }
    },
    methods: {
        onEditorReady(editor) {
            ...
            if (this.success_story) {
                this.form.detail = this.success_story.detail
            }
        },
        save() {
                ...
                onSuccess: (page) => {
                    if (Object.keys(page.props.errors).length == 0) {
                        if (this.method.toLowerCase() == 'put') {
                            this.$toast.success('Success Story is updated successfully!')
                        } else {
                            this.$toast.success('Success Story is created successfully!')
                        }
                        this.form.errors = [];
                        this.form.reset();
                    } else {

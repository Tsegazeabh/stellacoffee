<template>
    <div class="flex flex-wrap w-full">
        <div class="breadcrump">
            <ul>
                ...
                <li>{{_trans('label.shared.City')}}</li>
            </ul>
        </div>
        <div class="flex flex-wrap w-full justify-end">
            <inertia-link type="button" :href="route('city-creation-page')"
                          class="border text-gray-500 px-4 py-2 uppercase text-sm">
                <i class="fa fa-plus"></i>&nbsp;&nbsp;{{_trans('label.shared.Create City')}}
            </inertia-link>
        </div>
    ...
    <table class="cms-table">
        ...
        <tr>
            ...
            <th class="text-left p-3">City(EN)</th>
            <th class="text-left p-3">ከተማ(አማ)</th>
            <th class="text-left p-3">Ville(Fr)</th>
            <th class="text-left p-3">Città(IT)</th>
        ...
        <tr v-for="(content,index) in searchResult.data" :key="content.id">
            ...
            <td class="text-right">
                <inertia-link type="button"
                              ...
                              :href="route('city-editor-page', content.id)"
        ...
        methods: {
            forceDelete(content_id) {
                if (confirm("Are you sure you want to delete the content?")) {
                    Inertia.delete(route('delete-city', content_id), {
                        preserveScroll: true,
                        onSuccess: () => {
                            this.$toast.success('City is deleted successfully!');
                        },
                    });
                }
            },
            ...
            search() {
                this.form.submit('post', route('search-city'));
            }
        }
    }
</script>

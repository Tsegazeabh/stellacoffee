<template>
    <div class="flex flex-wrap w-full">
        <div class="breadcrump">
            <ul>
                ...
                <li>{{_trans('label.shared.Country')}}</li>
            </ul>
        </div>
        <div class="flex flex-wrap w-full justify-end">
            <inertia-link type="button" :href="route('country-creation-page')"
                          class="border text-gray-500 px-4 py-2 uppercase text-sm">
                <i class="fa fa-plus"></i>&nbsp;&nbsp;Create Country
            </inertia-link>
        </div>
    ...
    <table class="cms-table">
        ...
        <tr>
            ...
            <th class="text-left p-3">Country(EN)</th>
            <th class="text-left p-3">አገር(አማ)</th>
            <th class="text-left p-3">Nom du pays(Fr)</th>
            <th class="text-left p-3">Nome del Paese(IT)</th>
        ...
        <tr v-for="(content,index) in searchResult.data" :key="content.id">
            ...
            <td class="text-right">
                <inertia-link type="button"
                              ...
                              :href="route('country-editor-page', content.id)"
        ...
        methods: {
            forceDelete(content_id) {
                if (confirm("Are you sure you want to delete the content?")) {
                    Inertia.delete(route('delete-country', content_id), {
                        preserveScroll: true,
                        onSuccess: () => {
                            this.$toast.success('Country is deleted successfully!');
                        },
                    });
                }
            },
            ...
            search() {
                this.form.submit('post', route('search-country'));
            }
        }
    }
</script>

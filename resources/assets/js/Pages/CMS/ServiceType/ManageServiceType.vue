<template>
    <div class="flex flex-wrap w-full">
        <div class="breadcrump">
            <ul>
                ...
                <li>{{_trans('label.shared.Service Types')}}</li>
            </ul>
        </div>
        <div class="flex flex-wrap w-full justify-end">
            <inertia-link type="button" :href="route('service-type-creation-page')"
                          class="border text-gray-500 px-4 py-2 uppercase text-sm">
                <i class="fa fa-plus"></i>&nbsp;&nbsp;{{_trans('label.shared.Create Service Type')}}
            </inertia-link>
        </div>
    ...
    <table class="cms-table">
        ...
        <tr>
            ...
            <th class="text-left p-3">Service Type(EN)</th>
            <th class="text-left p-3">የአገልግሎት አይነት(አማ)</th>
            <th class="text-left p-3">Type De Service(FR)</th>
            <th class="text-left p-3">Tipo Di Servizio(IT)</th>
        ...
        <tr v-for="(content,index) in searchResult.data" :key="content.id">
            ...
            <td class="text-right">
                <inertia-link type="button"
                              ...
                              :href="route('service-type-editor-page', content.id)"
        ...
        methods: {
            forceDelete(content_id) {
                if (confirm("Are you sure you want to delete the content?")) {
                    Inertia.delete(route('delete-service-type', content_id), {
                        preserveScroll: true,
                        onSuccess: () => {
                            this.$toast.success('Service Type is deleted successfully!');
                        },
                    });
                }
            },
            ...
            search() {
                this.form.submit('post', route('search-service-type'));
            }
        }
    }
</script>

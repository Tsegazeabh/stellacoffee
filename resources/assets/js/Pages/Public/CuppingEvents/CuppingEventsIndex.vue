<template>
    ...
    <div class="w-full">
        <div v-if="result && result.total>0" class="flex flex-wrap">
            <div v-for="(event,index) in result.data" class="card-container flex flex-col md:flex-row w-full">
                <div class="grid grid-cols-3 justify-center items-stretch border-b my-4 pb-5">
                    <div class="col-span-1">
                        <img :src="event.contentable.first_image['src']" class="object-fill"/>
                    </div>
                    <inertia-link :href="event.url"
                                  class="col-span-2 px-10 flex flex-col justify-center items-start hover:bg-gray-200">
                        <h2 class="text-stella text-xl my-3">
                            {{ event.contentable.title }}
                        </h2>
                        <p class="text-justify">{{ event.contentable.lead_paragraph }}</p>
                          <div class="py-2 justify-center">
                           <template v-if="event.contentable.video_link">
                               <youtube-player class="w-100 h-100"
                                                ref="youtube"
                                                :videoid="_youTubeGetID(event.contentable.video_link)"
                                                :loop="loop"
                                                @ended="onEnded"
                                                @paused="onPaused"
                                                @played="onPlayed"
                               ...
                               <p class="text-stella text-lg my-3 font-bold text-right bottom-0 right-0"><a :href="event.contentable.video_link" target="_blank">{{_trans('label.shared.Video Link')}}</a></p>
                            </template>
                        </div>
                    </inertia-link>
                </div>

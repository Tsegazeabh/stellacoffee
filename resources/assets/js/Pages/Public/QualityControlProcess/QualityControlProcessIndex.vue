<template>
    ...
    <div class="w-full">
        <div v-if="result && result.total>0" class="flex flex-wrap">
            <div v-for="(quality,index) in result.data" :key="quality.id" class="card-container flex flex-col md:flex-row w-full">
                <div class="grid grid-cols-3 justify-center items-center border-b my-4 pb-5">
                    <div class="col-span-1">
                        <img :src="quality.contentable.first_image['src']" class="object-fill"/>
                    </div>
                    <div class="col-span-2 px-10 flex flex-col justify-center items-start">
                        <h2 class="text-stella text-xl my-3">
                            <inertia-link :href="quality.url">
                                {{quality.contentable.title}}
                            </inertia-link>
                        </h2>
                        <p class="text-justify">{{ quality.contentable.lead_paragraph }}</p>
                        <div class="py-2 justify-center">
                            <template  v-if="quality.contentable.video_link">
                                <youtube-player class="w-100 h-100"
                                                ref="youtube"
                                                :videoid="_youTubeGetID(quality.contentable.video_link)"
                                                :loop="loop"
                                                @ended="onEnded"
                                                @paused="onPaused"
                                                @played="onPlayed"
                                ...
                                <p class="text-stella text-lg my-3 font-bold text-right bottom-0 right-0"><a :href="quality.contentable.video_link" target="_blank">{{_trans('label.shared.Video Link')}}</a></p>
                            </template>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    export default defineComponent({
        name: "quality-control-process-index",
        ...
        provide: {
            menu_name:'Roasting',
            sub_menu_name:'Quality Control Process',
        },
        methods: {
            formatDate(date) {

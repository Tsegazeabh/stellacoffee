<template>
    ...
    <div class="w-full">
        <div v-if="result && result.total>0" class="flex flex-wrap">
            <div v-for="service in result" class="card-container flex flex-col md:flex-row w-full">
                <div class="grid grid-cols-3 justify-center items-center border-b my-4 pb-5">
                    <div class="col-span-1">
                        <img :src="service.image" class="object-fill"/>
                    </div>
                    <div class="col-span-2 px-10 flex flex-col justify-center items-start">
                        <h2 class="text-stella text-xl my-3">{{ service.name }}</h2>
                        <p>{{ service.description }}</p>
                        <span class="text-stella text-xl my-3">{{ service.price }}</span>
                    </div>
                </div>
            </div>
        </div>

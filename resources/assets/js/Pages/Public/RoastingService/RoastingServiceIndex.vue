<template>
    ...
    <div class="w-full">
        <div v-if="result && result.total>0" class="flex flex-wrap">
            <div v-for="(service,index) in result.data" :key="service.id" class="card-container flex flex-col md:flex-row w-full">
                <div class="grid grid-cols-3 justify-center items-center border-b my-4 pb-5">
                    <div class="col-span-1">
                        <img :src="service.contentable.first_image['src']" class="object-fill"/>
                    </div>
                    <div class="col-span-2 px-10 flex flex-col justify-center items-start">
                        <h2 class="text-stella text-xl my-3">
                            <inertia-link :href="service.url">
                                {{service.contentable.title}}
                            </inertia-link>
                        </h2>
...
<script>
    ...
    export default defineComponent({
        ...
        provide: {
            menu_name:'Roasting',
            sub_menu_name:'Roasting Services',
        },
        methods: {
            formatDate(date) {
